...
create function get_books_by_slug(user_id integer, book_slug character varying)
    returns TABLE(id integer, discount double precision, image character varying, is_bestseller smallint, popularity double precision, price integer, slug character varying, title character varying, pub_date date, code character varying, rate numeric, description text, user_rating integer, count1 bigint, count2 bigint, count3 bigint, count4 bigint, count5 bigint, rate_review integer, download_count integer)
    language plpgsql
as
 '
BEGIN
    return query
        ...
        where b.slug = $2
        group by b.id, b.discount, b.image, b.is_bestseller, b.popularity,
                 b.price, b.slug, b.title, b.pub_date, b.code, br.rating, b.code, b.rate, b1.description, download_count;
end
 ';
