create function find_books(user_id integer, search character varying, bestseller boolean, disc boolean, from_date date, to_date date)
    returns TABLE(id integer, discount double precision, image character varying, image_id integer, is_bestseller smallint, popularity double precision, price integer, slug character varying, title character varying, pub_date date, code character varying, rate numeric)
    language plpgsql
as
'
BEGIN
        return query select b.id,
                     ...
                     group by b.id, b.discount, b.image, b.image_id, b.is_bestseller, b.popularity, b.price, b.slug, b.title,
